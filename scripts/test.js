(function () {
    const Test = {
        progressBarElement: null, // создаем в начале этот элемент, чтобы он у нас существовал, потом заполняем
        nextButtonElement: null, //создаем в начале этот элемент, чтобы он у нас существовал
        prevButtonElement: null, //создаем в начале этот элемент, чтобы он у нас существовал
        passButtonElement: null, //создаем в начале этот элемент, чтобы он у нас существовал
        optionsElement: null,  //блок с ответами, который изначально отсутствует
        questionTitleElement: null, // название вопроса выводим на страницу, изначально ноль - этого элемента нет и мы не можем его использовать
        quiz: null,  //свойство нашего главного объекта - данные которые пришли в тесте после запроса (const xhr) разместим в новую переменную
        currentQuestionIndex: 1, // индекс ТЕКУЩЕГО вопроса (полученного с сервера), (но первый у нас будет с индексом -1)
        userResult: [], //массив для сохранения выбранных пользователем ответов, после нажатия на кнопку ДАЛЕЕ после каждого вопроса, в Ф move()
